<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>
    <style>
        * {
            font-family: 'Helvetica neue', 'Helvetica', 'Figtree', sans-serif;
        }

        h1 {
            font-weight: 700;
            line-height: 32px;
            font-size: 24px;
            text-align: center;
            margin-top: 80px;
        }

        p.service {
            margin-bottom: 45px;
            margin-left: 20px;
        }

        .bordered-gray {
            border-style: solid;
            border-width: 1px;
            border-color: #1f2937;
            padding-left: 12px;
            padding-right: 12px;
            padding-top: 4px;
            padding-bottom: 4px;
        }

        table {
            border-width: 1px !important;
            border-color: #1f2937;
            border-collapse: collapse;
            width: 100%;
        }

        .pxpy {
            padding-left: 12px;
            padding-right: 12px;
            padding-top: 4px;
            padding-bottom: 4px;
        }
    </style>
</head>

<body>
    <div>
        <div style="display:inline-block; float: left">
            <img style="max-width: 100px;"
                src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAARwAAAEcCAMAAAAiKvvSAAAABGdBTU
              "
                alt="logo" />
            <div style="margin-top: -100px; margin-left: 7rem">
                372, av. Colonel Mondjiba <br />
                Commune de Ngaliema <br />
                Kinshasa
            </div>
        </div>
        <div style="float: right; margin-left:20rem;">
            <p>{{ $req->user->compte->city }}, le {{ date('d/m/Y') }}</p>
        </div>
    </div>
    <br>
    <br>
    <br>
    <div style="margin-top: 80px; margin-bottom: 20px;">
        <h1>
            BON DE LIVRAISON DE LA REQUISITION INTERNE
        </h1>
        <p class="service" style="font-size: 18px; margin-top: 16px; line-height: 28px">
            <span style="font-weight: 600">Service demandeur :</span> <span
                style="font-weight: 700">{{ $req->service }}</span>
        </p>
        <p class="service" style="font-size: 15px;">
            <span style="font-weight: 600">Requisition N° :</span> <span
                style="font-weight: 700">{{ $req->numero }}</span>
        </p>
    </div>
    <table>
        <tr>
            <th class="bordered-gray">N°</th>
            <th class="bordered-gray">DESIGNATION</th>
            <th class="bordered-gray">QTE DEMANDEE</th>
            <th class="bordered-gray">QTE LIVREE</th>
        </tr>
        @foreach ($req->deliveries as $key => $item)
            <tr class="items">
                <td class="bordered-gray" style="text-align: center"> {{ $key + 1 }}</td>
                <td class="bordered-gray"> {{ $item['designation'] }} </td>
                <td class="bordered-gray" style="text-align: right"> {{ $item['to_deliver'] }} </td>
                <td class="bordered-gray" style="text-align: right">{{ $item['delivered'] }} </td>
            </tr>
        @endforeach
    </table>
</body>

</html>
